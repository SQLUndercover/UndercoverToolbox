...
--ASCII representation of 'DataRelay'
SELECT ASCII('D')



--XOR The ASCII code for 'G' with a simple key of 123
SELECT CHAR(ASCII('D') ^ 123)
...
--To encrypt 'DataRelay'

EXEC EncryptData_XOR 'DataRelay',23


--To decrypt


EXEC EncryptData_XOR 'SvcvEr{vn',23
...
EXEC EncryptData_XOR 'SvcvEr{vn', @Key
SET @Key = @Key + 1
END
...
SELECT CHAR(ASCII('D') + 2)


--To decrypt, reverse the processs
SELECT CHAR(ASCII('F') - 2)



--combine XOR and Caesar, first apply XOR and then shift 
SELECT CHAR((ASCII('D') + 2) ^ 123)


--to decrypt
SELECT CHAR((ASCII('=') ^ 123) - 2)
...
--encrypt
EXEC EncryptData_CaesarXOR @PlainText = 'DataRelay', @Key = 210, @Shift = 2, @Decrypt = 0



--decrypt
EXEC EncryptData_CaesarXOR @PlainText = '”±¤±†µ¼±©', @Key = 210, @Shift = 2, @Decrypt = 1
...
-----------------------------------------------------------------------------------------------
--to encrypt....
EXEC EncryptData_MiniDES 4027558485, 'DataRelay'


--to decrypt...
EXEC EncryptData_MiniDES 4027558485, 'TqŠŸBu’Ÿi0ÞÞ'



--but what about if we try a plain text attack?
EXEC Crack_O_Matic 'Data', 'TqŠŸ'




--let's try using that key....
EXEC EncryptData_MiniDES 269549310, 'TqŠŸBu’Ÿi0ÞÞ'